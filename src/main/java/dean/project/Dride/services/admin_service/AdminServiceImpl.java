...
public class AdminServiceImpl implements AdminService {
    ...
    public GlobalApiResponse sendInviteRequests(InviteAdminRequest invitation) {
        ...
        String response = sendAdminMail(request, admin);
    ...
    private String sendAdminMail(EmailNotificationRequest request, Admin admin) {
        Long userId = admin.getUser().getId();
        String adminName = admin.getUser().getName();
        String link = jwtUtil.generateVerificationLink(userId);

        context.setVariables(Map.of("name", adminName, "verifyUrl", link));

        String htmlContent = templateEngine.process("admin_mail", context);
    ...
    private Admin currentAdmin() {
        return getAdminByUserId(currentUser().getId())
    ...
    public AdminDTO getCurrentAdmin() {
        return modelMapper.map(currentAdmin(), AdminDTO.class);
    }
